...
pub fn read_semantic_outlines(config: &Config, files: Vec<String>) -> String {
    let mut out =
        String::from("SEMANTIC FILE OUTLINES (Read these to understand the structure):\n");
    ...
    for file_path in files {
        ...
        match fs::read_to_string(&full_path) {
            Ok(content) => {
                ...
                if chunks.is_empty() {
                    out.push_str(&format!("\n--- FILE: {} (Unsupported language or no structure found. {} bytes) ---\n", file_path, content.len()));
                } else {
                    out.push_str(&format!("\n--- FILE: {} ---\n", file_path));
                    for chunk in chunks {
                        out.push_str(&format!(
        ...
        Ok(content) => {
            ...
            for target_name in chunk_names {
                if let Some(chunk) = chunks.iter().find(|c| c.name == target_name) {
                    out.push_str(&format!(
                        "\n--- {} [{}] (Lines {} - {}) ---\n{}\n",
                        chunk.name, chunk.kind, chunk.start_line, chunk.end_line, chunk.content
                    ));
                } else {
                    out.push_str(&format!(
